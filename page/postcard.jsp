...
<body>
  ...
  <%
  String sql5="SELECT * FROM product WHERE id=5";
  ...
  <div class="row">
    ...
    <div class="right">
      ...
      <br>
      <h3 style="font-size: 16px;">把想對你說的話寫在溫潤厚磅的紙上，帶著祝福一起傳遞出去吧！</h3><br>
      <form style="font-size: 20px; font-weight: bold;">
        款式：<select size="1" name="color" style="font-size: 20px; width: 200px;">
          <option> 請選擇款式</option>
          <option> 重要的日子</option>
          <option> 想說的話藏不住</option>
          <option> 哇嘎哩共</option>
          <option> 有些話只想對你說</option>
          <option> 飛到你身邊</option>
        </select>
      </form><br>
  ...
  <div class="list" id="商品內容">
    <br>
    <h2>商品內容</h2>
    <h3>重要的日子、想說的話藏不住、哇嘎哩共、有些話只想對你說、飛到你身邊、五入一組</h3>
    <h2> 規格</h2>
    <h3 id="tab1">尺寸：14.8 × 10.5 cm<br>
      材質：12oz帆布<br>
      顏色：360sm<br>
      特色：義大利粉面彩柔，正反面燙黑壓凹。<br>
  ...
  <%
             sql="SELECT * FROM `guestbook` WHERE `Product`='對你說明信片'"; 
             ...
             sql="SELECT * FROM `guestbook` WHERE `Product`='對你說明信片' ORDER BY `GBNO` DESC LIMIT ";
             sql+=start_record+",5";
